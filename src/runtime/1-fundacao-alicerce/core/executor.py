"""
MESH - Executor - Zero-Trust Execution
Este executor NUNCA pode validar a própria proposta.
Regra: Se proposed_by == executor_id, rejeita.
"""
...
class Executor:
    ...
    def execute(self, proposal: Dict[str, Any], proposed_by: str) -> Dict[str, Any]:
        if proposed_by == self.executor_id:
            return {"success": False, "reason": "Zero-trust: executor não pode validar própria proposta", "rollout_reward": 0}
        print(f"⚙️ Executor {self.executor_id} executando: {proposal.get('payload', '')}")
...
if __name__ == "__main__":
    ...
    print(f"Teste 1 (válido): {r1}")
    r2 = exec1.execute(prop, proposed_by="executor-35b")
    print(f"Teste 2 (inválido - mesmo ID): {r2}")
